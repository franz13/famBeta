/*
  # Family Member Management Functions and Policies
  
  1. Policies
    - Add RLS policies for family members table
    - Enable viewing and adding family members
  
  2. Helper Functions
    - Add validation function for family member data
    - Add function to check family member limits
  
  3. Main Function
    - Add function to handle family member creation
*/
...
CREATE FUNCTION public.validate_family_member(
  p_family_id UUID,
  p_email TEXT,
  p_role TEXT
) RETURNS VOID AS $$
...
CREATE FUNCTION public.check_family_member_limit(
  p_family_id UUID
) RETURNS VOID AS $$
BEGIN
